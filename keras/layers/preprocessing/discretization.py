...
class Discretization(base_preprocessing_layer.PreprocessingLayer):
  ...
  def adapt(self, data, batch_size=None, steps=None):
    ...
    Calling `adapt` on a `Discretization` layer is an alternative to passing in
    a `bin_boundaries` argument during construction. A `Discretization` layer
    should always be either adapted over a dataset or passed `bin_boundaries`.

    During `adapt`, the layer will estimate the quantile boundaries of the input
    dataset. The number of quantiles can be controlled via the `num_bins`
